...
if [[ "$masterb" =  True ]]
 then
    ...
    cockroach start --insecure --advertise-host=$pub --background --http-port=80 --store=$store --locality=country=$country,state="$state",city=$city,region=$region --join=$master  >> /home/cockroachdb 
fi
